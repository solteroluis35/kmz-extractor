...
# Reference points
reference_points = np.array([[255, 255, 255, 78], [255, 225, 255, 73], [255, 200, 255, 68],
                            [255, 128, 255, 63], [
...
for i in np.arange(0, 16):
    ...
    def rgb_to_dbz(red, green, blue):
        ...
        if red <= 30:
            ...
            if la_index == 0:
                assigned_reflectivity = reference_points[11,
                                                         3] - (lineal_factors[11]) * la_distance
            elif la_index == 5:
                assigned_reflectivity = reference_points[16,
                                                         3] + (lineal_factors[15]) * la_distance
            else:
                if lb_index == la_index:
                    assigned_reflectivity_with_la_distance = reference_points[11+la_index, 3] - \
                        (lineal_factors[11 + la_index]) * la_distance
                    assigned_reflectivity_with_lb_distance = reference_points[12+la_index, 3] + \
                        (lineal_factors[11 + la_index]) * lb_distance
                    assigned_reflectivity = (
                        assigned_reflectivity_with_la_distance +
                        assigned_reflectivity_with_lb_distance)/2
                else:
                    assigned_reflectivity_with_la_distance = reference_points[11+la_index, 3] + \
                        (lineal_factors[10 + la_index]) * la_distance
                    assigned_reflectivity_with_lb_distance = reference_points[10+la_index, 3] - \
        ...
        if red >= 225:
            ...
            if la_index == 10:
                assigned_reflectivity = reference_points[10,
                                                         3] + (lineal_factors[9]) * la_distance
            elif la_index == 0:
                assigned_reflectivity = reference_points[0,
                                                         3] - (lineal_factors[0]) * la_distance
            else:
                if lb_index == la_index:
                    assigned_reflectivity_with_la_distance = reference_points[la_index, 3] - (
                        lineal_factors[la_index]) * la_distance
                    assigned_reflectivity_with_lb_distance = reference_points[la_index+1, 3] + (
    ...
    for i in np.arange(0, np.size(filtered_image_array, 0)):
        dbz_matrix[i, 0] = rgb_to_dbz(
            filtered_image_array[i, 0], filtered_image_array[i, 1], filtered_image_array[i, 2])
